...
# --- 嘗試引入 KPI 計算函式，若找不到模組或找不到資料檔案，則使用預設數值（防錯機制） ---
# 注意：這裡改成 except Exception（原本是 except ModuleNotFoundError）。
# 因為 utils/kpi.py 現在已經存在，若只攔截 ModuleNotFoundError，
# 一旦 Datas 底下的 CSV 檔案缺少導致 calc_otd() 等函式拋出
# FileNotFoundError，會沒有被攔截到，造成整個網站崩潰而不是顯示預設值。
try:
